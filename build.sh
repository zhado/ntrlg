...
if [ "$1" = "buildrun" ]; then
	...
	gcc ./src/main.cpp $links $flags -ggdb3 -O0 -o deb_out && gdb ./deb_out $2
elif [ "$1" = "build" ]; then
	gcc ./src/main.cpp $links $flags -o out $2
elif [ "$1" = "debbuild" ]; then
	gcc ./src/main.cpp $links $flags -ggdb3 -O0 -o deb_out $2
elif [ "$1" = "buildrunfast" ]; then
	if [ "$(echo $ANDROID_DATA)" != "" ]; then
		echo "on android"
		sed -i '/my_port 1901/a ip 192.168.50.103' serv_conf 
		gcc ./src/main.cpp $links $flags -O3 -D ANDROID -o out && ./out $2
	else
	...
	if [ "$(echo $ANDROID_DATA)" != "" ]; then
		echo "on android"
		sed -i '/my_port 1901/a ip 192.168.50.103' serv_conf 
		gcc ./src/main.cpp $links $flags -O3 -ggdb3 -D ANDROID -o deb_out && gdb ./deb_out $2
	else
		gcc ./src/main.cpp $links $flags -O3 -ggdb3 -o deb_out && gdb ./deb_out $2
	fi
else 
	echo "unknown option"
	exit 1
